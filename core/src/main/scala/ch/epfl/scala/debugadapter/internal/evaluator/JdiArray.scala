...
import com.sun.jdi.{ArrayReference, IntegerValue, ObjectReference, ReferenceType, ThreadReference, Value, VirtualMachine}
...
object JdiArray {
  ...
  private def newInteger(value: Int, classLoader: JdiClassLoader, thread: ThreadReference): Option[JdiObject] = {
    val vm = thread.virtualMachine()
